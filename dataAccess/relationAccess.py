...
class relationAccess(DataAccess):
    ...
    def delete_relation(self, id, user, ip):
        ...
        self.db[self.collectionName].update_one({'_id': ObjectId(id)}, {'$set': {'status': 'deleted', 'modifyUser':user}})
    ...
    def get_relation_keyList(self, queryType, key):
        if queryType == 'reason':
            print('/^'+key+'/')
            return self.db[self.collectionName].aggregate([{'$project': {'reason': 1}}, {'$group': {'_id': '$reason'}}, {'$match': {'_id': {'$regex': '^'+key}}}])

        return self.db[self.collectionName].aggregate([{'$project': {"combined":  {'$setUnion': ["$subjects."+queryType, "$objects."+queryType]}}}, {"$unwind": "$combined"}, {"$group": {"_id": "$combined"}}, {"$match": {"_id": {'$regex': '^'+key}}}])
    ...
    def download_relations(self):
        return self.db[self.collectionName].aggregate([{"$unwind": "$subjects"}, {"$unwind": "$objects"}, {'$project': {
            "subjects": 1,
            "reason": 1,
            "objects.name": 1,
            "objects.idNumber": 1,
    ...
    def get_connections(self, idNumber):
        return self.db[self.collectionName].aggregate([
    ...
    def check_duplicate(self, target , id):
        return self.db[self.collectionName].find({target+'.idNumber': id}).count()
